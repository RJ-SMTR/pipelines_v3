...
11. Run the bundled script once with `--dry-run` and review the output:
   ...
   ```bash
   python .claude/skills/smtr-github-subissues/scripts/create_subissues.py --parent <issue-number> --plan <plan.json> --dry-run
   ```
...
13. If the user asked to actually create issues, run the same command without `--dry-run` and include the chosen assignee mode:
   ...
   ```bash
   python .claude/skills/smtr-github-subissues/scripts/create_subissues.py --parent <issue-number> --plan <plan.json> --assignee-mode parent-fallback-viewer
   ```
