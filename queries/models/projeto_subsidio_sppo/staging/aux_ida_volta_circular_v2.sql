...
select
    *,
    lead(datetime_partida) over (
        partition by id_veiculo, servico_realizado, shape_id_planejado
...
where sentido = "C" and (data >= date("{{ var('DATA_SUBSIDIO_V24_INICIO') }}") or data between date('2025-05-01') and date('2025-05-31'))
