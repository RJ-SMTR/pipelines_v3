...
with
    viagem as (
        select
            *,
            datetime_diff(datetime_chegada, datetime_partida, minute)
            + 1 as tempo_viagem
        from
            (
                select *
                from
                    (
                        ...
                        from {{ ref("aux_viagem_circular") }} v
                ...
                (
                    ...
                    from
                        (
                            ...
                            -- por shape (ida/volta)
                            -- distancia_planejada,
                            -- round(distancia_planejada + lead(distancia_planejada)
                            -- over (
...
select distinct
    ...
    round(
        100 * d.distancia_aferida / v.distancia_planejada, 2
    ) as perc_conformidade_distancia,
