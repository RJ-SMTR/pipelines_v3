...
with
    gps_agregado as (
        select
            ...
            count(*) over (
                partition by servico_jae, id_validador
            ...
            ) over (partition by servico_jae, id_validador)
            as qtde_registros_gps_georreferenciados,
            row_number() over (
                partition by servico_jae, id_validador order by datetime_gps
            ) as rn
        from
            (
                select
                ...
                where data = current_date("America/Sao_Paulo") and modo = "VLT"
            )
        where rn = 1
    )
select
