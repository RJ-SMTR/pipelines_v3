...
with
    ...
    dados_novos as (
        select
            ...
            case
                when a.indicador_nao_inclusao is true then true
        ...
        left join aux_retorno_negativacao r
            on a.id_auto_infracao = r.id_auto_infracao
    ...
    sha_dados_atuais as (
        ...
        {% else %}
            select
                cast(null as string) as id_auto_infracao,
    ...
    sha_dados_completos as (
        select 
            n.*, 
            a.* except (id_auto_infracao)
        from sha_dados_novos n
        left join sha_dados_atuais a using (id_auto_infracao)
    ),
    ...
    dados_novos_com_controle as (
        select
            ...
            data_inclusao,
            data_baixa,
            coalesce(data_confirmacao_inclusao_nova, data_confirmacao_inclusao_atual) as data_confirmacao_inclusao,
            coalesce(data_confirmacao_baixa_nova, data_confirmacao_baixa_atual) as data_confirmacao_baixa,
            indicador_nao_inclusao,
            motivo,
            nome,
            cpf,
    ...
    final as (
        ...
        {% if is_incremental() and partitions | length > 0 %}
            ...
            where 
                data in ({{ partitions | join(", ") }})
                and id_auto_infracao not in (select id_auto_infracao from dados_novos_com_controle)
        {% endif %}
    )
