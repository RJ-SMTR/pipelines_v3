{{
    config(
        ...
        alias="ordem_pagamento_entidade_dia",
    )
}}
...
with
    dados_novos as (
        select
            ...
            ) as id_ordem_pagamento_entidade_dia,
            v.cnpj as cnpj_entidade,
            count(1) as quantidade_verificacao_valida,
            count(distinct cpf_guardador_veiculo) as quantidade_guardador_ativo,
            sum(v.valor_repasse) as valor_repasse_entidade
        from
            {{ verificacao_guardador_veiculo_riorotativo }},
            unnest(valor_repasse_entidades) as v
        where
            date(datetime_inclusao_verificacao) < current_date("America/Sao_Paulo")
            and indicador_verificacao_valida
    ...
    {% if ordem_table_exists %}
        ...
        particoes_completas as (
            select
                ...
                ifnull(
                    n.id_ordem_pagamento_entidade_dia, a.id_ordem_pagamento_entidade_dia
                ) as id_ordem_pagamento_entidade_dia,
                ifnull(n.cnpj_entidade, a.cnpj_entidade) as cnpj_entidade,
                ifnull(
                    n.quantidade_verificacao_valida, a.quantidade_verificacao_valida
                ) as quantidade_verificacao_valida,
                ifnull(
                    n.quantidade_guardador_ativo, a.quantidade_guardador_ativo
                ) as quantidade_guardador_ativo,
                ifnull(
                ...
                case
                    when
                        a.id_ordem_pagamento_entidade_dia is not null
                        and a.valor_repasse_entidade != n.valor_repasse_entidade
                    then
                        {% if ignorar_diferenca_ordem %} 1 / 1
                        {% else %} 1 / 0
                        {% endif %}
                ...
                case
                    when
                        max(a.id_ordem_pagamento_entidade_dia) over (
                            partition by ifnull(n.data_ordem, a.data_ordem)
                        )
                        is not null
                        and a.id_ordem_pagamento_entidade_dia is null
                    then
                        {% if ignorar_diferenca_ordem %} 1 / 1
                        {% else %} 1 / 0
                        {% endif %}
                end as novo_registro_ordem
            from dados_novos n
            full outer join dados_atuais a using (id_ordem_pagamento_entidade_dia)
        ),
    {% else %}
        particoes_completas as (
            select
    ...
    sha_dados_novos as (
        select
            * except (
                quantidade_valida_divergente, valor_divergente, novo_registro_ordem
            ),
    ...
    sha_dados_atuais as (
        {% if ordem_table_exists %}
            ...
            select
                id_ordem_pagamento_entidade_dia,
        ...
        {% else %}
            select
                cast(null as string) as id_ordem_pagamento_entidade_dia,
    ...
    sha_dados_completos as (
        select n.*, a.* except (id_ordem_pagamento_entidade_dia)
        from sha_dados_novos n
        left join sha_dados_atuais a using (id_ordem_pagamento_entidade_dia)
    ),
