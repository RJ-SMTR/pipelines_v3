...
{#
  Classificação de viagens (veículo + autuação + tecnologia).
  Espelho de viagem_classificada; cadeia legada permanece intacta.

  WIP/teste: base temporária = viagem_completa (servico_realizado).
  Trocar de volta para viagem_valida quando houver dados.
#}
...
with
    viagens as (
        select
            ...
            distancia_planejada,
            sentido
        from {{ ref("viagem_completa") }}  -- WIP: voltar para viagem_valida
    ...
    viagem_status as (
        select
            ...
            v.sentido,
            "Ônibus SPPO" as modo,  -- viagem_completa não tem modo; voltar coalesce c/ viagem_valida
            ve.tecnologia,
            ve.status,
            ve.indicadores
        from viagens v
