...
{#
  Bilhetagem / Sem transação sobre viagem_valida_regularidade_temperatura.
  Adaptação de viagem_transacao_aux_v2; persiste indicadores no JSON.
#}
{% set interval_minutes = 30 %}
{% set date_range_start %}"{{ var('date_range_start') }}"{% endset %}
{% set date_range_end %}"{{ var('date_range_end') }}"{% endset %}
...
with
    ...
    viagem as (
        select
            ...
            sentido,
            distancia_planejada
        from {{ ref("viagem_valida_regularidade_temperatura") }}
        where
    ...
    bilhetagem as (
        select
            ...
            v.servico,
            v.ano_fabricacao,
            v.indicador_regularidade_ar_condicionado_viagem,
            any_value(v.indicadores) as indicadores,  -- JSON não pode ir no GROUP BY
            case
                when
            ...
            va.indicador_sem_transacao,
            va.indicador_estado_equipamento_aberto,
            max(
                eep.indicador_gps_servico_divergente
            ) as indicador_gps_servico_divergente,
            v.datetime_partida,
            v.datetime_chegada
        ...
        json_set(
            json_set(
                json_set(
                    ...
                    '$.indicador_estado_equipamento_aberto.valor',
                    indicador_estado_equipamento_aberto
                ),
                '$.quantidade_transacao.valor',
                coalesce(quantidade_transacao, 0)
            ),
            '$.quantidade_transacao_riocard.valor',
            coalesce(quantidade_transacao_riocard, 0)
        ),
        '$.indicador_gps_servico_divergente.valor',
        coalesce(indicador_gps_servico_divergente, false)
    ) as indicadores,
    servico,
    sentido,
    distancia_planejada,
    coalesce(quantidade_transacao, 0) as quantidade_transacao,
    coalesce(quantidade_transacao_riocard, 0) as quantidade_transacao_riocard,
    coalesce(indicador_sem_transacao, false) as indicador_sem_transacao,
    indicador_estado_equipamento_aberto,
