...
- Altera materialização do modelo `aux_gps_filtrada` de `ephemeral` para `incremental` (particionada por `data`, clusterizada por `datetime_gps`) quando `15_minutos = false`, eliminando leituras repetidas dos dados brutos de GPS nos modelos `gps`, `aux_gps_velocidade`, `aux_gps_parada`, `aux_gps_trajeto_correto` e no teste `check_gps_treatment` (redução de ~66% dos bytes processados por materialização horária)
- Adiciona filtro de partição nas leituras de `aux_gps_filtrada` nos modelos `gps`, `gps_15_minutos`, `aux_gps_velocidade`, `aux_gps_parada` e `aux_gps_trajeto_correto`
- Remove colunas não utilizadas e torna incondicional o filtro de partição na CTE `gps_filtrada` do teste `check_gps_treatment`
- Adiciona `aux_gps_filtrada` ao seletor `gps`
...
- Migração dos modelos `gps_validador.sql`, `gps_validador_van.sql`, `gps_validador_aux.sql` e `staging_gps_validador.sql` para o repositório v3 (https://github.com/RJ-SMTR/pipelines_v3/pull/42)
